...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val searchTerms: Observable[String] = Observable(searchTermField.text).sanitized
	...
	case Failure(f) => status.text = f.toString
      }
    }

    // TO IMPLEMENT
    val selections: Observable[String] = Observable(suggestionList.selection.items: _*)
