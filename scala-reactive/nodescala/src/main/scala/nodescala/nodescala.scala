...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    Future.run() {
      ct => {
	Future {
	  def repeat(next: Future[(Request, Exchange)]): Unit = {
	    if (ct.nonCancelled) {
	      next.onComplete {
		case Success((req, xchg)) => {
		  respond(xchg, ct, handler(req))
		  repeat(listener.nextRequest)
		}
		case Failure(f) => repeat(listener.nextRequest)
	      }
	    }
	  }
	  repeat(listener.nextRequest)
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      createContext(xchg => p success (xchg.request, xchg))
      p.future
    }
  }
