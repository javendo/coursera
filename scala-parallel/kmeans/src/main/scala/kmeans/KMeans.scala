...
class KMeans {
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    def updateMeans(means: GenSeq[Point]) = {
      val newMeans = update(classify(points, means), means)
      println(s"newMeans: $newMeans, converged: ${converged(eta)(means, newMeans)}")
      (newMeans, converged(eta)(means, newMeans))
    }
    def _kMeans(pair: (GenSeq[Point], Boolean)): GenSeq[Point] = pair match {
      case (means, true) => means
      case (means, false) => _kMeans(updateMeans(means))
    }
    _kMeans(updateMeans(means))
  }
}
