...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    downsweepSequential(input, output, 0f, 1, input.length)
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def _upsweepSequential(idx: Int, max: Float): Float = {
      if (idx >= 0 && idx < until) _upsweepSequential(idx + 1, Math.max(input(idx) / idx, max)) else max
    }
    _upsweepSequential(from, 0)
  }
