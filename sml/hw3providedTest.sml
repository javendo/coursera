    ...
    val tests = [
        ...
        ("first_answer", [
            assert("should raise NoAnswer for empty list",
                ...
                case SOME(SOME(first_answer (fn x => NONE) [])) handle NoAnswer => NONE 
								    |  _ => SOME(NONE) of
                    NONE => true
                 |  SOME _ => false
            ),
            assertEqualsInt("for equals_3 on increasing list", 3,
                first_answer (fn x => case x of 3 => SOME 3 
					     |  _ => NONE) [1, 2, 3, 4, 5]
            )
        ]),
        ("all_answers", [
        ...
        ("match", [
            ...
            assertEquals("binding list for matching TupleP", SOME [], match(Tuple[Const 17, Unit, Const 4, Constructor("egg", Const 4), Constructor("egg", Constructor("egg",Const 4))], Tuple[Wildcard,Wildcard]))
        ]),
        ("first_match", [
