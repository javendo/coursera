...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = if (that.isEmpty) this else this.incl(that.head).union(that.tail)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = this.tail.filter0(p, if (p(this.head)) accu.incl(this.head) else accu)
  def ascendingByRetweet0(accu: Trending): Trending = {
    val minTweet = this.findMin; this.remove(minTweet).ascendingByRetweet0(accu + minTweet)
  }
...
class NonEmptyTrending(elem: Tweet, next: Trending) extends Trending {
   ...
   */
  def + (tw: Tweet): Trending = new NonEmptyTrending(elem, next + tw)
  def head: Tweet = elem
  def tail: Trending = next
  def isEmpty: Boolean = false
  override def toString = "NonEmptyTrending(" + elem.retweets + ", " + next + ")"
}
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(e => t.text.contains(e)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(e => t.text.contains(e)))
