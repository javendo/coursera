...
fun shift (dx,dy,e) =
	...
	NoPoints => e
      ...
      | _ => raise Impossible("The program should not reach this point")
...
fun compare_point (x1,y1) (x2,y2) =
    let
	val same = real_close_point (x1,y1) (x2,y2)
	val p1_less_than_p2 = (not (real_close (x1,x2)) andalso x1 < x2) orelse
			      (real_close(x1,x2) andalso not (real_close (y1,y2)) andalso y1 < y2)
    in
	if same
	then 0
	else if p1_less_than_p2
	then 1
	else ~1
    end
...
fun preprocess_prog (e) =
	...
	NoPoints => e
      ...
      | Line _   => e
      | VerticalLine _ => e
      | LineSegment(x1,y1,x2,y2) => (case compare_point (x1,y1) (x2,y2) of
					 0 => Point(x1,y1)
				       | 1 => LineSegment(x1,y1,x2,y2)
				       | ~1 => LineSegment(x2,y2,x1,y1)
				       | _ => raise Impossible("The program should not reach this point"))
      | Var _ => e
